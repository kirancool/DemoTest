var nvd3Charts = function() {
	...
	var startChart1 = function() {
		...
		nv.addGraph(function() {
			...
			.axisLabel('Country').tickFormat(d3.format(',r'));

			chart.yAxis//Chart y-axis settings
			.axisLabel('count').tickFormat(d3.format('.02f'));
	...
	var startChart3 = function() {
		...
		d3.json('http://localhost:8080/stocks/getStockPriceOpen', function(data) {
			nv.addGraph(function() {
			...
			d3.json('assets/plugins/nvd3/cumulativeLineData.json', function(data) {
				nv.addGraph(function() {
					function pieData(data){
						...
						{
							if(data[i]['stock_price_low'] != null){
								temp.push(data[i]['stock_price_low']);
								data1[data[i]['stock_price_low']] = data[i]['count'];
							}
						}
					}
